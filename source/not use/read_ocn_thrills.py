...
def read_ocn_thrills():
    ...
    now_ocn_html = requests.get("http://ocnthrills.tving.com/ocnthrills/schedule?startDate="+now_day)
    now_ocn_soup = BeautifulSoup(now_ocn_html.text, 'html5lib')

    tomo_ocn_html = requests.get("http://ocnthrills.tving.com/ocnthrills/schedule?startDate="+tomo_day)
...
if __name__ == '__main__':
    airTime_ocnT_list, program_ocnT_list = read_ocn_thrills()
